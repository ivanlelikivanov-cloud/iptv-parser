...
# ==================== ИСТОЧНИКИ ====================
STATIC_SOURCES = [
    # iptv-org: страна и основной язык
    "https://iptv-org.github.io/iptv/countries/ru.m3u",
    "https://iptv-org.github.io/iptv/languages/rus.m3u",
    # Языки народов России (кириллица пройдёт фильтр)
...
# Резервные регионы РФ по часовым поясам (если API iptv-org недоступен)
FALLBACK_REGIONS = [
    "ru-kgd",                          # Калининград      (MSK-1)
    "ru-mow", "ru-mos", "ru-spe",      # Москва, Питер    (MSK)
    "ru-len", "ru-kda", "ru-ros",
    "ru-vgg", "ru-sta", "ru-da",
    "ru-sam", "ru-ud", "ru-ta",        # Самара, Казань   (MSK+1)
    "ru-ba", "ru-udm",
    "ru-per", "ru-sve", "ru-che",      # Урал             (MSK+2)
    "ru-tyu",
    "ru-oms", "ru-nvs", "ru-tom",      # Зап. Сибирь      (MSK+3)
    "ru-kem", "ru-alt",
    "ru-kya", "ru-irk", "ru-bu",       # Вост. Сибирь     (MSK+4/5)
    "ru-sa", "ru-zab",                 # Якутия           (MSK+6)
    "ru-pri", "ru-kha", "ru-amu",      # Дальний Восток   (MSK+7)
    "ru-sak", "ru-mag",                #                  (MSK+8)
    "ru-kam", "ru-chu",                # Камчатка         (MSK+9)
]

# ==================== НАСТРОЙКИ МОЩНОСТИ ====================
MAX_CHANNELS_TO_PARSE = 6000   # Большой пул, чтобы после фильтра осталось 500-800+
SOURCE_WORKERS = 20            # Потоки загрузки источников
CHECK_WORKERS = 60             # Потоки проверки каналов
CHECK_TIMEOUT = 4.0            # Дольше 4 секунд — канал в мусорку
UPDATE_EVERY = 86400           # Самообновление раз в 24 часа
...
VALID_CONTENT_TYPES = ('video/', 'mpegurl', 'octet-stream', 'audio/', 'mp2t')

# ==================== СЕССИИ (connection pooling) ====================
...
def fetch_ru_regions():
    """Автообнаружение ВСЕХ регионов РФ через API iptv-org (все часовые пояса)"""
    try:
        ...
        logger.info(f"API iptv-org: найдено регионов РФ: {len(urls)}")
        return urls
    except Exception:
        return []
...
def fetch_source_text(url):
    try:
        r = get_session().get(url, timeout=10, headers=HEADERS_WEB, verify=False)
...
# ==================== ЖЁСТКАЯ ПРОВЕРКА (<= 4 сек) ====================
def check_one(url):
    """Канал жив только если ответил за <= 4 сек и отдаёт видеоконтент"""
    session = get_session()
    ...
    try:
        ...
        if elapsed() <= CHECK_TIMEOUT and r.status_code < 400:
            ct = r.headers.get('content-type', '').lower()
            if any(v in ct for v in VALID_CONTENT_TYPES):
                return True
    except Exception:
        pass

    # Этап 2: GET только с ОСТАТКОМ времени из 4 секунд
...
def parse_m3u(text, raw, seen_urls):
    """Возвращает True, если достигнут лимит каналов"""
    current_inf = ''
    current_name = ''
...
def update_cache():
    ...
    is_updating = True
    start = time.time()
    logger.info("🔄 Старт: сбор источников по всем часовым поясам РФ...")

    try:
        # 1. Собираем источники: статика + регионы из API + резерв + скрапинг
        ...
        logger.info(f"Всего уникальных источников: {len(sources)}")

        # 2. Параллельно качаем все плейлисты
        texts = []
        ...
        # 3. Парсим и дедуплицируем
        raw = []
        seen = set()
        for txt in texts:
            if parse_m3u(txt, raw, seen):
                break
        logger.info(f"Уникальных русских каналов: {len(raw)}. Жёсткая проверка (<= 4 сек)...")

        # 4. Параллельная проверка каждого канала
        alive = []
        ...
        # 5. Сортировка по категориям и именам
        ...
        lines = [
            ...
            '# Живых каналов: ' + str(len(alive)) + ' | отклик каждого <= 4 сек',
            '# Регионы: все часовые пояса РФ (MSK-1 .. MSK+9)',
        ...
        logger.info(f"✅ Готово: {len(alive)} живых каналов из {len(raw)} за {elapsed:.0f} сек")
...
def home():
    ...
    <p>Все часовые пояса РФ | отклик &le; 4 сек | обновление раз в 24 ч</p>
