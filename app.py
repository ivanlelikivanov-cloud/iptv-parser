...
# ==================== РАСШИРЕННЫЙ ПАРСИНГ ====================
def extract_m3u_links(html, base_url):
    """Извлекает .m3u/.m3u8 ссылки из HTML с обработкой относительных путей"""
    links = set()
    
    # Паттерны для поиска
    patterns = [
        r'(https?://[^\s"\'<>]+?\.m3u8?)',  # Полные URL
        r'["\'](/[^"\']*?\.m3u8?)["\']',     # Относительные пути
        r'href=["\']([^"\']*?\.m3u8?)["\']', # href атрибуты
        r'url=["\']([^"\']*?\.m3u8?)["\']',  # url параметры
    ]
    ...
    for pattern in patterns:
        ...
        for match in matches:
            if match.startswith('http'):
                # Полная ссылка
                if any(x in match.lower() for x in ['ru', 'russia', 'moscow', 'spb', 'sib', 'ural', 'volga', 'kazan', 'sochi']):
                    links.add(match)
            elif match.startswith('/'):
                # Относительный путь — делаем абсолютным
                parsed = urlparse(base_url)
                absolute = f"{parsed.scheme}://{parsed.netloc}{match}"
                if 'ru' in absolute.lower():
                    links.add(absolute)
    
    return list(links)

def fetch_dynamic_sources(page_urls):
    """Сканирует сайты и собирает .m3u/.m3u8 ссылки"""
    dynamic = set()
    ...
    for page in page_urls:
        try:
            ...
            if r.status_code == 200:
                links = extract_m3u_links(r.text, page)
                for link in links:
                    # Дополнительная проверка на дубликаты и валидность
                    if link not in DIRECT_SOURCES and 'iptv' in link.lower():
                        dynamic.add(link)
                logger.info(f"✅ +{len(links)} ссылок с {page.split('/')[2]}")
...
stats = {"total": 0, "direct": 0, "dynamic": 0, "sources": 0}
...
def update_cache():
    ...
    all_sources = list(set(DIRECT_SOURCES + dynamic_sources))
    
    lines = [
        "#EXTM3U",
        f"# 🇷🇺 IPTV Russia Pro — {time.strftime('%Y-%m-%d %H:%M')}",
        f"# Прямых источников: {len(DIRECT_SOURCES)}",
        f"# Найдено динамически: {len(dynamic_sources)}",
        "# Только русские каналы | Автообновление каждые 30 мин"
    ]
    ...
    logger.info(f"✅ {count} каналов | Прямые: {direct_count} | Динамика: {dynamic_count} | Источников: {ok_sources}")
...
.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;max-width:700px;margin:20px auto}}
...
<div class="card"><div class="stat" style="color:#58a6ff">{s['direct']}</div><div>Прямых</div></div>
...
🔍 Парсинг сайтов: {len(HTML_SOURCES)}<br>
🇷🇺 Только русские каналы с кириллицей
</small>
</body></html>"""
...
if __name__ == '__main__':
    logger.info(f"🚀 Запуск... {len(DIRECT_SOURCES)} прямых + {len(HTML_SOURCES)} сайтов для парсинга")
